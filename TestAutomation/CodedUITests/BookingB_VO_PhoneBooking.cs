...
namespace TestAutomation
{
    ...
    /// </summary>
    [CodedUITest]
    public class BookingB_VO_PhoneBooking:logWriter
    {
        //Booking Flow: Product Page >>Villa Only HS page>>Pay By Phone Booking//
        public BookingB_VO_PhoneBooking()
        {
        }
        
        [TestMethod]
        public void BookingB_VO_PhoneorCnfrmBtn()
        {
            ...
            string sqlQuery = "SELECT TOP " + noOfVillas + " VillaId FROM tblVillaAvailability va  INNER JOIN dbo.TblVilla TV ON va.Villaid = TV.ID where va.available='A' and TV.MaxOccupancy = " + TotalPax + " ORDER BY NEWID();";
            dataReaderVillaID = getdata.getData(sqlQuery, InitialCatalog);
            while (dataReaderVillaID.Read())
            {
                villaID = dataReaderVillaID[0].ToString();
